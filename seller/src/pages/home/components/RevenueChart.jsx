import { useState, useMemo } from "react";
import { useExchangeRate } from "@/hooks/useExchangeRate";

export default function RevenueChart({ orders = [] }) {
  const [currency, setCurrency] = useState("NGN");
  const rate = useExchangeRate();
  const totalRevenue = orders.reduce((s, o) => s + (o.amount_ngn || 0), 0);
  const data = useMemo(() => {
    const map = {};
    orders.forEach((o) => {
      const d = new Date(o.created_at);
      const key = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}`;
      const val = currency === "NGN" ? (o.amount_ngn || 0) : Math.round((o.amount_ngn || 0) * rate);
      map[key] = (map[key] || 0) + val;
    });
    const keys = Object.keys(map).sort().slice(-7);
    return keys.map((k) => ({ month: k.split("-")[1], value: map[k] }));
  }, [orders, currency, rate]);
  const maxValue = Math.max(...data.map((d) => d.value), 1);
  const formatValue = (v) => {
    if (v >= 1000000) return `${(v / 1000000).toFixed(1)}M`;
    if (v >= 1000) return `${(v / 1000).toFixed(0)}k`;
    return v;
  };

  return (
    <div className="bg-white rounded-xl border border-gray-100 p-5">
      {/* Header */}
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-base font-bold text-gray-900" style={{ fontFamily: "'Poppins', sans-serif" }}>Revenus mensuels</h3>
          <p className="text-[10px] text-gray-400 mt-0.5">
            Total : <span className="font-semibold text-[#125C8D]">
              {currency === "NGN" ? `₦${totalRevenue.toLocaleString()}` : `${Math.round(totalRevenue * rate).toLocaleString()} FCFA`}
            </span>
          </p>
        </div>

        <div className="flex items-center bg-gray-100 rounded-full p-0.5">
          {["NGN", "FCFA"].map((c) => (
            <button
              key={c}
              onClick={() => setCurrency(c)}
              className={`text-xs font-semibold px-3 py-1.5 rounded-full transition-all whitespace-nowrap cursor-pointer ${
                currency === c ? "bg-white text-[#125C8D] shadow-sm" : "text-gray-500 hover:text-gray-700"
              }`}
            >
              {c}
            </button>
          ))}
        </div>
      </div>
      {/* Chart */}
      <div className="flex items-end gap-2" style={{ height: "120px" }}>
        {data.map((item, idx) => {
          const heightPct = maxValue > 0 ? (item.value / maxValue) * 100 : 0;
          const isLast = idx === data.length - 1;
          return (
            <div key={item.month + idx} className="flex-1 flex flex-col items-center gap-1 h-full group relative">
              <div className="absolute -top-7 left-1/2 -translate-x-1/2 bg-gray-900 text-white text-[10px] font-semibold px-2 py-0.5 rounded whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none transition-all z-10">
                {currency === "NGN" ? "₦" : ""}{formatValue(item.value)}{currency === "FCFA" ? " F" : ""}
              </div>
              <div className="flex-1 flex items-end w-full">
                <div className={`w-full rounded-t-lg transition-all duration-500 ${isLast ? "bg-[#125C8D]" : "bg-[#125C8D]/20"}`} style={{ height: `${heightPct}%`, minHeight: "4px" }}></div>
              </div>
              <span className="text-[9px] text-gray-400 font-medium">{item.month}</span>
            </div>
          );
        })}
        {data.length === 0 && <p className="text-xs text-gray-400 w-full text-center py-8">Aucune donnée</p>}
      </div>
      <div className="flex items-center gap-4 mt-4 pt-4 border-t border-gray-100">
        <div>
          <p className="text-[10px] text-gray-400 uppercase tracking-widest">Commandes</p>
          <p className="text-sm font-bold text-gray-900">{orders.length}</p>
        </div>
        <div>
          <p className="text-[10px] text-gray-400 uppercase tracking-widest">Moy. mensuelle</p>
          <p className="text-sm font-bold text-gray-900">{data.length > 0 ? Math.round(totalRevenue / data.length / 1000) : 0}k</p>
        </div>
        <div>
          <p className="text-[10px] text-gray-400 uppercase tracking-widest">Meilleur mois</p>
          <p className="text-sm font-bold text-gray-900">
            {data.length > 0 ? data.reduce((a, b) => a.value > b.value ? a : b).month : "—"}
          </p>
        </div>
      </div>
    </div>
  );
}
