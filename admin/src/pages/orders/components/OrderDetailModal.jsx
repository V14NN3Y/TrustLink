...
export default function OrderDetailModal({ order, onClose, onUpdate, onDispatchSuccess }) {
  const [tab, setTab] = useState('info');
  const [editStep, setEditStep] = useState(order.journey_step || 'awaiting_payment');
  ...
  const tabs = [
    ...
    ...(canDispatch ? [{ key: 'dispatch', label: 'Dispatch', icon: 'ri-send-plane-line' }] : []),
  ...
  return (
    <>
          ...
          <div className="flex-1 overflow-y-auto p-6">
            {tab === 'info' && (
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  { label: 'Montant FCFA', value: formatXOF(order.amount_xof) },
            ...
            {tab === 'dispatch' && canDispatch && (
              <div className="space-y-5">
                <div>
                  <label className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2 block">Assigner au vendeur</label>
                  <select value={selectedSellerId} onChange={e => setSelectedSellerId(e.target.value)} className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm outline-none focus:border-trustblue cursor-pointer bg-white">
                    <option value="">Choisir un vendeur…</option>
                    {sellers.map(s => (
                      <option key={s.id} value={s.id}>{s.name} — {s.hub} ({s.category})</option>
                    ))}
                  </select>
                </div>
                <div>
                  <label className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2 block">Instructions pour le vendeur</label>
                  <textarea
                    value={dispatchInstructions}
                    onChange={e => setDispatchInstructions(e.target.value)}
                    maxLength={200}
                    rows={3}
                    placeholder="Instructions optionnelles pour le vendeur..."
                    className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm outline-none focus:border-trustblue resize-none"
                  />
                  <p className="text-xs text-slate-400 text-right mt-0.5">{dispatchInstructions.length}/200</p>
                </div>
                <button
                  onClick={() => setShowDispatchModal(true)}
                  disabled={!selectedSellerId}
                  className="w-full py-2.5 text-white rounded-xl font-semibold text-sm cursor-pointer disabled:opacity-50 flex items-center justify-center gap-2"
                  style={{ backgroundColor: '#125C8D' }}
                >
                  <i className="ri-send-plane-line" />Valider & Dispatcher
                </button>
              </div>
            )}
      ...
      {showDispatchModal && selectedSeller && (
        <DispatchModal
          order={order}
          ...
          onConfirm={() => {
            ...
            if (onDispatchSuccess) onDispatchSuccess(order.id);
          }}
