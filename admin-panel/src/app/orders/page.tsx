...
export default function OrdersPage() {
      ...
      const { data: disputeData } = await supabase
        ...
        .select(`
          *,
          order:orders (
            *,
            client:profiles!orders_client_id_fkey (first_name, last_name, city)
          )
        `)
        .eq('status', 'OPEN')
        .order('created_at', { ascending: false })
  ...
  const handleResolveDispute = async (status: 'RESOLVED_REFUND' | 'RESOLVED_RELEASE') => {
    ...
    if (disputeError) {
      alert('Error resolving dispute: ' + disputeError.message);
      return;
    }

    // Update order status based on resolution
    const orderStatus = status === 'RESOLVED_REFUND' ? 'PENDING' : 'DELIVERED'; // Simplified
    await supabase
      .from('orders')
      .update({ status: orderStatus, escrow_locked: status === 'RESOLVED_REFUND' })
      .eq('id', activeDispute.order_id);

    alert(`Litige résolu avec succès : ${status === 'RESOLVED_REFUND' ? 'Client remboursé' : 'Vendeur payé'}`);
    setActiveDispute(null);
  };
    ...
    <AdminLayout>
      ...
      <div className="mb-6">
         <h2 className="text-xl font-extrabold text-foreground mb-4 flex items-center gap-2">
            ...
            <span className="w-5 h-5 bg-rose-500 text-white text-[10px] rounded-full flex items-center justify-center font-black animate-bounce shadow-lg shadow-rose-200">
              {activeDispute ? '1' : '0'}
            </span>
         </h2>
      </div>
      ...
      {!activeDispute ? (
        <div className="bg-white rounded-3xl p-12 text-center border border-dashed border-border mb-20">
           <p className="text-gray-400 font-medium italic">Aucun litige ouvert nécessitant un arbitrage immédiat.</p>
        </div>
      ) : (
        <div className="bg-white rounded-3xl shadow-2xl shadow-rose-200/40 border border-rose-100 overflow-hidden mb-20 animate-in fade-in zoom-in duration-700">
          <div className="bg-rose-50/80 backdrop-blur-md p-8 border-b border-rose-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <div>
                  <h2 className="text-xl font-black text-rose-800 tracking-tight flex items-center gap-2">
                     ...
                     LITIGE CRITIQUE #{activeDispute.id.slice(0,8).toUpperCase()}
                  </h2>
                  <p className="text-sm text-rose-600 font-medium mt-1">
                    {(() => {
                      const client = Array.isArray(activeDispute.order?.client) ? activeDispute.order.client[0] : activeDispute.order?.client;
                      return (
                        <>
                    ...
                    ID Commande: {activeDispute.order_id.slice(0,8)} • 
                    Valeur: {activeDispute.order?.total_cfa?.toLocaleString()} CFA
                  </p>
              </div>
          ...
          <div className="p-8">
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-10">
                  ...
                  <div className="border border-border rounded-[2.5rem] overflow-hidden bg-gray-50 shadow-inner">
                      <div className="px-6 py-4 bg-white border-b border-border text-[10px] font-black text-gray-500 flex justify-between items-center uppercase tracking-widest">
                          <span>1. Vidéo Inspection Lagos (Agent)</span>
                          <span className="text-primary">{activeInspection ? new Date(activeInspection.created_at).toLocaleDateString() : 'N/A'}</span>
                      </div>
                      ...
                      <div className="p-5 bg-white text-xs font-bold leading-relaxed">
                          <span className="text-green-600 uppercase tracking-widest border-b-2 border-green-100 pb-0.5 inline-block mb-3 italic">Notes Inspection</span>
                          <p className="text-gray-500 italic">"{activeInspection?.notes || 'Aucune note d\'inspection trouvée.'}"</p>
                      </div>
                  </div>
                  ...
                  <div className="border border-rose-200 rounded-[2.5rem] overflow-hidden bg-rose-100 shadow-inner ring-4 ring-rose-50/50">
                      <div className="px-6 py-4 bg-white border-b border-rose-100 text-[10px] font-black text-rose-500 flex justify-between items-center uppercase tracking-widest">
                          ...
                          <span className="text-rose-600">{new Date(activeDispute.created_at).toLocaleDateString()}</span>
                      </div>
                      <div className="aspect-video bg-slate-900 group flex items-center justify-center relative overflow-hidden">
                          <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
                          <span className="text-slate-500 text-sm font-black uppercase tracking-widest z-10">Evidence Client</span>
                      ...
                      <div className="p-5 bg-white text-xs font-bold leading-relaxed">
                          <span className="text-rose-600 uppercase tracking-widest border-b-2 border-rose-100 pb-0.5 inline-block mb-3 italic">Motif du Litige</span>
                          <p className="text-gray-500 italic">"{activeDispute.reason || 'Le client n\'a pas fourni de description textuelle.'}"</p>
                      </div>
                  </div>
              </div>
              ...
              <div className="mt-12 pt-8 border-t border-gray-100 flex flex-wrap justify-end gap-5">
                  ...
                  <button 
                    onClick={() => handleNotifySeller(activeDispute.order_id)}
                    className="px-8 py-3.5 bg-white border border-accent text-accent rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-accent/5 transition-all shadow-sm"
                  >
                      Notifier Vendeur
                  </button>
                  <button 
                    onClick={() => handleResolveDispute('RESOLVED_REFUND')}
                    className="px-8 py-3.5 bg-white border-2 border-rose-600 text-rose-600 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-rose-50 transition-all shadow-lg shadow-rose-100"
                  >
                      Rembourser via Escrow
                  </button>
                  <button 
                    onClick={() => handleResolveDispute('RESOLVED_RELEASE')}
                    className="px-10 py-3.5 bg-primary text-white rounded-2xl font-black text-xs uppercase tracking-[0.2em] shadow-xl shadow-primary/30 hover:bg-primary-dark hover:-translate-y-1 transition-all"
                  >
